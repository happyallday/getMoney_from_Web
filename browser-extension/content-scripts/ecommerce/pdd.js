...
(function() {
    ...
    function initPDD() {
        if (window.WelfareHelperContent) {
            ...
            setupPDDSpecificFeatures();
            monitorPDDWelfare();
        }
    }

    function setupPDDSpecificFeatures() {
        setInterval(() => {
            detectPDDCheckIn();
    ...
    function detectPDDCheckIn() {
        ...
        pageButtons.forEach(button => {
            ...
            if (checkInIndicators.some(indicator => buttonText.includes(indicator))) {
                if (!button.dataset.pddHandled) {
                    addPDDHelperButton(button, '签到');
                    button.dataset.pddHandled = 'true';
                }
            }
        });
    }
    ...
    function detectCouponCollection() {
        ...
        pageButtons.forEach(button => {
            ...
            if (couponIndicators.some(indicator => buttonText.includes(indicator))) {
                if (!button.dataset.pddCouponHandled) {
                    addPDDHelperButton(button, '领券');
                    button.dataset.pddCouponHandled = 'true';
                }
            }
        });
    }
    ...
    function detectActivityParticipation() {
        ...
        pageButtons.forEach(button => {
            ...
            if (activityIndicators.some(indicator => buttonText.includes(indicator))) {
                if (!button.dataset.pddActivityHandled) {
                    addPDDHelperButton(button, '活动');
                    button.dataset.pddActivityHandled = 'true';
                }
            }
        });
    }

    function addPDDHelperButton(element, action) {
        ...
        helperBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            handlePDDAction(element, action);
        });
    ...
    function handlePDDAction(element, action) {
        try {
    ...
    function monitorCashValue() {
        setInterval(() => {
            ...
            cashSelectors.forEach(selector => {
                const element = document.querySelector(selector);
                if (element) {
                    const value = element.textContent.match(/\d+\.?\d*/);
                    if (value) {
                        logCashUpdate(value[0]);
                    }
                }
            });
        }, 30000);
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initPDD);
    } else {
        initPDD();
    }
